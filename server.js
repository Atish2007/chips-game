...
app.post('/api/register', async (req, res) => {
  ...
  if (normalizedUsername.length < 3 || normalizedUsername.length > 20) return res.status(400).json({ error: 'Username must be 3-20 characters' });
  if (password.length < 6) return res.status(400).json({ error: 'Password must be at least 6 characters' });
  if (users[normalizedUsername]) return res.status(400).json({ error: 'Username already exists' });
  try {
    const hashedPassword = await bcrypt.hash(password, 10);
    users[normalizedUsername] = { password: hashedPassword, wins: 0, losses: 0, activeRoom: null, createdAt: new Date().toISOString(), email: email || null, friends: [], friendRequests: [] };
    saveUsers(users);
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, username: normalizedUsername, stats: { wins: user.wins, losses: user.losses }, activeRoom: user.activeRoom, email: user.email || null, needsEmail: !user.email });
...
app.get('/api/user/:username', (req, res) => {
  ...
  if (user.activeRoom && !rooms[user.activeRoom]) { user.activeRoom = null; saveUsers(users); }
  res.json({ username: normalizedUsername, stats: { wins: user.wins, losses: user.losses }, activeRoom: user.activeRoom, email: user.email || null, needsEmail: !user.email });
});
...
app.post('/api/update-email', (req, res) => {
  ...
  saveUsers(users);
  console.log(`[Email] Saved for ${normalizedUsername}: ${email}`);
  res.json({ success: true });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('disconnect', () => {
    ...
    if (room.state === 'LOBBY') {
      ...
      if (room.players.length === 0) { delete rooms[socket.roomCode]; } else { socket.to(room.code).emit('player_left', { username: player.username }); broadcastPlayersList(room); if (room.owner === player.username && room.players.length > 0) { room.owner = room.players[0].username; broadcastPlayersList(room); } }
      return;
    }
...
server.listen(PORT, '0.0.0.0', () => { console.log(`🚀 Server on port ${PORT}`); console.log(` Users: ${USERS_FILE}`); });
