...
function loadUsers() {
  ...
  } catch (error) { console.error('[DB] Error loading users:', error); }
  return {};
}

function saveUsers(users) {
  try { fs.writeFileSync(USERS_FILE, JSON.stringify(users, null, 2), 'utf8'); } 
  catch (error) { console.error('[DB] Error saving users:', error); }
}
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    // Check if email is missing (for old accounts)
    const needsEmail = !user.email;
    
    res.json({ 
      success: true, username: normalizedUsername,
      stats: { wins: user.wins, losses: user.losses },
      activeRoom: user.activeRoom, email: user.email || null, needsEmail: needsEmail
    });
...
app.get('/api/user/:username', (req, res) => {
  ...
  if (user.activeRoom && !rooms[user.activeRoom]) { user.activeRoom = null; saveUsers(users); }
  res.json({
    username: normalizedUsername, stats: { wins: user.wins, losses: user.losses },
    activeRoom: user.activeRoom, email: user.email || null, needsEmail: !user.email
  });
});
...
app.post('/api/clear-active-room', (req, res) => {
  ...
  const normalizedUsername = username.toLowerCase();
  if (users[normalizedUsername]) { users[normalizedUsername].activeRoom = null; saveUsers(users); }
...
app.post('/api/send-friend-request', (req, res) => {
  ...
  if (normalizedUser === normalizedFriend) return res.status(400).json({ error: 'Cannot add yourself' });
  ...
  if (users[normalizedUser].friends.includes(normalizedFriend)) return res.status(400).json({ error: 'Already friends' });
  if (users[normalizedFriend].friendRequests.includes(normalizedUser)) return res.status(400).json({ error: 'Request already sent' });
...
app.post('/api/handle-friend-request', (req, res) => {
  const { username, requestFrom, action } = req.body; // action: 'accept' or 'decline'
  if (!username || !requestFrom || !action) return res.status(400).json({ error: 'Missing fields' });
  const normalizedUser = username.toLowerCase();
  ...
  if (reqIndex === -1) return res.status(400).json({ error: 'Request not found' });
  ...
  if (action === 'accept') {
    ...
    if (!users[normalizedUser].friends.includes(normalizedFrom)) users[normalizedUser].friends.push(normalizedFrom);
    if (!users[normalizedFrom].friends.includes(normalizedUser)) users[normalizedFrom].friends.push(normalizedUser);
  }
  saveUsers(users);
  res.json({ success: true });
});
...
app.post('/api/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"Chips Game" <${process.env.GMAIL_USER}>`, to: user.email, subject: 'Password Reset Code',
...
app.post('/api/reset-password', async (req, res) => {
  ...
  try {
    ...
    saveUsers(users); delete passwordResetTokens[normalizedUsername];
    res.json({ success: true });
  ...
  if (connectedUsers[username]) {
    ...
    if (user && user.friends) {
      user.friends.forEach(friend => {
        if (connectedUsers[friend]) io.to(connectedUsers[friend].socketId).emit('friend_status_update', { username, status });
      });
    }
  }
}
...
io.on('connection', (socket) => {
  socket.on('user_logged_in', ({ username }) => {
    ...
    if (user && user.friends) {
      user.friends.forEach(friend => {
        if (connectedUsers[friend]) io.to(connectedUsers[friend].socketId).emit('friend_status_update', { username, status: 'main' });
      });
    }
  });
  ...
  socket.on('reconnect_to_game', ({ roomCode, username }) => {
    ...
    if (!room) return socket.emit('reconnect_failed', { message: 'Lobby not found' });
    const player = room.players.find(p => p.username === username);
    if (!player) return socket.emit('reconnect_failed', { message: 'Not in this lobby' });
    ...
    socket.join(roomCode); socket.roomCode = roomCode; socket.username = username;
    ...
    socket.emit('game_state_restored', {
      state: room.state, players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })),
      owner: room.owner, myHealth: room.health[socket.id] || 3, currentTurnUsername: room.players.find(p => p.id === room.currentTurn)?.username,
      myEatenChips: room.eatenChips[socket.id] || [], roomCode: room.code, myUsername: username, timeLeft
    });
  });
  ...
  socket.on('create_room', (username) => {
    ...
    rooms[roomCode] = { code: roomCode, owner: username, players: [{ id: socket.id, username, ready: false }], state: 'LOBBY', poisons: {}, health: {}, eatenChips: {}, currentTurn: null, playAgainVotes: {}, reconnectTimeout: null, poisonStartTime: null, poisonDuration: 30, poisonPaused: false, poisonPausedAt: null };
    socket.join(roomCode); socket.roomCode = roomCode; socket.username = username;
    if (users[username]) { users[username].activeRoom = roomCode; saveUsers(users); }
    updateFriendStatus(username, 'room');
    socket.emit('room_created', { roomCode, owner: username });
    io.to(roomCode).emit('update_players_list', { players: rooms[roomCode].players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === username })), owner: username });
  });
  ...
  socket.on('join_room', ({ roomCode, username }) => {
    ...
    if (users[username]) { users[username].activeRoom = roomCode; saveUsers(users); }
    updateFriendStatus(username, 'room');
    emitToRoom(room, 'player_joined', { players: room.players.map(p => ({ id: p.id, username: p.username })), owner: room.owner });
    io.to(roomCode).emit('update_players_list', { players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })), owner: room.owner });
  });

  socket.on('player_ready', () => {
    const room = rooms[socket.roomCode]; if (!room) return;
    const player = room.players.find(p => p.id === socket.id); if (player) player.ready = !player.ready;
    io.to(room.code).emit('update_players_list', { players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })), owner: room.owner });
  });
  ...
  socket.on('start_game', () => {
    ...
    room.state = 'POISON_PHASE'; room.poisonStartTime = Date.now(); room.poisonDuration = 30;
    emitToRoom(room, 'start_poison_phase', { duration: 30 });
  });

  socket.on('submit_poisons', ({ poisonedChips }) => {
    const room = rooms[socket.roomCode]; if (!room || room.state !== 'POISON_PHASE') return;
    ...
    if (Object.keys(room.poisons).length === 2) {
      ...
      setTimeout(() => {
        ...
        room.players.forEach(p => { room.health[p.id] = 3; room.eatenChips[p.id] = []; });
        emitToRoom(room, 'coin_toss_result', { firstPlayerId: room.currentTurn, firstPlayerName: room.players[firstPlayerIndex].username });
      }, 3000);
    }
  });
  ...
  socket.on('eat_chip', ({ chipIndex }) => {
    const room = rooms[socket.roomCode]; if (!room || room.state !== 'PLAYING' || room.currentTurn !== socket.id) return;
    if (!room.eatenChips[socket.id]) room.eatenChips[socket.id] = [];
    if (room.eatenChips[socket.id].includes(chipIndex)) return;

    const opponent = room.players.find(p => p.id !== socket.id);
    const isPoisoned = room.poisons[opponent.id].includes(chipIndex);
    room.eatenChips[socket.id].push(chipIndex);
    emitToRoom(room, 'chip_eaten', { chipIndex, playerId: socket.id, isPoisoned, message: isPoisoned ? 'You Got Poisened!' : 'You Are Safe!' });

    if (isPoisoned) {
      room.health[socket.id]--;
      if (room.health[socket.id] <= 0) {
        room.state = 'GAME_OVER';
        const winner = opponent.username, loser = socket.username;
  ...
  socket.on('play_again', () => {
    const room = rooms[socket.roomCode]; if (!room || room.state !== 'GAME_OVER') return;
  ...
  socket.on('leave_room', () => {
    ...
    const room = rooms[socket.roomCode];
    const playerIndex = room.players.findIndex(p => p.id === socket.id); if (playerIndex === -1) return;
    ...
    socket.roomCode = null; socket.username = null;
    updateFriendStatus(leavingPlayer.username, 'main');
    if (room.players.length === 0) { delete rooms[room.code]; } 
    else {
      socket.to(room.code).emit('player_left', { username: leavingPlayer.username });
      io.to(room.code).emit('update_players_list', { players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })), owner: room.owner });
      if (room.owner === leavingPlayer.username && room.players.length > 0) { room.owner = room.players[0].username; io.to(room.code).emit('update_players_list', { players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })), owner: room.owner }); }
    }
  });
  ...
  socket.on('disconnect', () => {
    if (socket.username) {
      ...
      if (user && user.friends) { user.friends.forEach(friend => { if (connectedUsers[friend]) io.to(connectedUsers[friend].socketId).emit('friend_status_update', { username: socket.username, status: 'offline' }); }); }
    ...
    if (room.state === 'LOBBY') {
      const playerIndex = room.players.findIndex(p => p.id === socket.id); if (playerIndex !== -1) room.players.splice(playerIndex, 1);
      ...
      else { socket.to(room.code).emit('player_left', { username: player.username }); io.to(room.code).emit('update_players_list', { players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })), owner: room.owner }); }
      return;
    }
    ...
    if (room.state !== 'GAME_OVER') {
      ...
      room.reconnectTimeout = setTimeout(() => {
        const opponent = room.players.find(p => p.id !== socket.id);
        if (opponent) { io.to(room.code).emit('opponent_lost', { message: `${player.username} disconnected and didn't return. You win!` }); if (users[opponent.username]) { users[opponent.username].wins++; users[opponent.username].activeRoom = null; saveUsers(users); } }
