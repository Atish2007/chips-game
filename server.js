...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    console.log(`[Verify] Code for ${normalizedUsername}: ${code}`);
    ...
    try {
      if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
        ...
        await transporter.sendMail({ 
          ...
          to: normalizedEmail, 
          subject: 'Verify your email - Chips Game', 
          html: `
            <div style="font-family: Arial; text-align: center; padding: 20px;">
              <h2 style="color: #667eea;">Welcome to Chips Game!</h2>
              <p>Your verification code is:</p>
              <h1 style="color: #f5576c; font-size: 48px; margin: 20px;">${code}</h1>
...
app.post('/api/verify-email', (req, res) => {
  ...
  if (!token) return res.status(400).json({ error: 'No verification code found. Please register again.' });
  ...
  if (token.code !== code) return res.status(400).json({ error: 'Invalid code. Please try again.' });
...
app.post('/api/resend-verification', (req, res) => {
  ...
  console.log(`[Resend] New code for ${normalizedUsername}: ${code}`);
  ...
  try {
    if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
      ...
      transporter.sendMail({ 
        from: `"Chips Game" <${process.env.GMAIL_USER}>`, 
        to: users[normalizedUsername].email, 
        subject: 'New verification code - Chips Game', 
        html: `
          <div style="font-family: Arial; text-align: center; padding: 20px;">
            <h2 style="color: #667eea;">Your new verification code:</h2>
            <h1 style="color: #f5576c; font-size: 48px; margin: 20px;">${code}</h1>
            <p>This code expires in 10 minutes.</p>
          </div>
        ` 
...
io.on('connection', (socket) => {
  ...
  socket.on('reconnect_to_game', ({ roomCode, username }) => {
    ...
    // FIX: Only resume poison if state is STILL poison phase
