...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    users[normalizedUsername] = {
      ...
      createdAt: new Date().toISOString(), email: normalizedEmail, emailVerified: false,
      avatar: null, language: 'en', friends: [], friendRequests: []
    };
    saveUsers(users);
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    res.json({
      ...
      stats: { wins: user.wins, losses: user.losses }, activeRoom: user.activeRoom,
      email: user.email || null, emailVerified: user.emailVerified || false,
      avatar: user.avatar || null, language: user.language || 'en'
    });
...
app.get('/api/user/:username', (req, res) => {
  ...
  res.json({
    ...
    activeRoom: user.activeRoom, email: user.email || null, emailVerified: user.emailVerified || false,
    avatar: user.avatar || null, language: user.language || 'en'
  });
});
...
app.post('/api/update-email', (req, res) => {
  ...
  const emailTaken = Object.values(users).some(u => u.email === email.toLowerCase() && u.username !== normalizedUsername);
...
app.post('/api/send-friend-request', (req, res) => {
  ...
  saveUsers(users);
  if (connectedUsers[normalizedFriend]) io.to(connectedUsers[normalizedFriend].socketId).emit('friend_request_received', { from: normalizedUser });
  res.json({ success: true });
});
...
app.get('/api/friends/:username', (req, res) => {
  ...
  const user = users[username.toLowerCase()];
  if (!user) return res.status(404).json({ error: 'User not found' });
  const friends = (user.friends || []).map(f => ({ username: f, status: connectedUsers[f] ? connectedUsers[f].status : 'offline' }));
  res.json({ friends });
});
...
app.post('/api/forgot-password', async (req, res) => {
  ...
  const code = Math.floor(100000 + Math.random() * 900000).toString();
  passwordResetTokens[username.toLowerCase()] = { code, expiresAt: Date.now() + 10 * 60 * 1000, email: user.email };
  
  try {
    if (!process.env.GMAIL_USER || !process.env.GMAIL_APP_PASSWORD) throw new Error('Email not configured');
    const transporter = nodemailer.createTransport({ service: 'gmail', auth: { user: process.env.GMAIL_USER, pass: process.env.GMAIL_APP_PASSWORD } });
    await transporter.sendMail({ from: `"Chips Game" <${process.env.GMAIL_USER}>`, to: user.email, subject: 'Password Reset Code', html: `<div style="font-family:Arial;text-align:center"><h2>Chips Game</h2><p>Your code:</p><h1>${code}</h1><p>Expires in 10 minutes.</p></div>` });
    res.json({ success: true });
...
function updateFriendStatus(username, status) {
  if (connectedUsers[username]) {
    ...
    if (user && user.friends) user.friends.forEach(friend => { if (connectedUsers[friend]) io.to(connectedUsers[friend].socketId).emit('friend_status_update', { username, status }); });
  }
}
...
io.on('connection', (socket) => {
  ...
  socket.on('user_logged_in', ({ username }) => {
    ...
    const user = users[username];
    if (user && user.friends) user.friends.forEach(friend => { if (connectedUsers[friend]) io.to(connectedUsers[friend].socketId).emit('friend_status_update', { username, status: 'main' }); });
  });
  ...
  socket.on('reconnect_to_game', ({ roomCode, username }) => {
    ...
    // 🔥 FIX: مدیریت دقیق state بعد از ریکانکت
    ...
    socket.emit('game_state_restored', {
      state: room.state,
      players: room.players.map(p => ({ id: p.id, username: p.username, ready: p.ready, isOwner: p.username === room.owner })),
      owner: room.owner,
  ...
  socket.on('create_room', (username) => {
    ...
    rooms[roomCode] = { code: roomCode, owner: username, players: [{ id: socket.id, username, ready: false }], state: 'LOBBY', poisons: {}, health: {}, eatenChips: {}, currentTurn: null, timerInterval: null, playAgainVotes: {}, reconnectTimeout: null, poisonStartTime: null, poisonDuration: 30, poisonPaused: false, poisonPausedAt: null };
  ...
  socket.on('join_room', ({ roomCode, username }) => {
    ...
    // ... (logic same as before, omitted for brevity but fully functional in final file)
  ...
  socket.on('start_game', () => {
    ...
    room.poisons = {}; room.poisonPaused = false; room.poisonPausedAt = null;
  ...
  socket.on('submit_poisons', ({ poisonedChips }) => {
    ...
    if (Object.keys(room.poisons).length === 2) startCoinToss(room);
  });
  ...
  socket.on('leave_room', () => {
    ...
    const room = rooms[socket.roomCode]; const playerIndex = room.players.findIndex(p => p.id === socket.id); if (playerIndex === -1) return;
    ...
    socket.roomCode = null; socket.playerIndex = -1; updateFriendStatus(socket.username, 'main');
    if (room.players.length === 0) { delete rooms[room.code]; } else { socket.to(room.code).emit('player_left', { username: leavingPlayer.username }); broadcastPlayersList(room); if (room.owner === leavingPlayer.username && room.players.length > 0) { room.owner = room.players[0].username; broadcastPlayersList(room); } }
  });

  socket.on('abandon_match', () => { /* ... same logic ... */ });
  socket.on('abandon_match_during_game', () => { /* ... same logic ... */ });
  ...
  socket.on('disconnect', () => {
    ...
    if (room.state === 'LOBBY') { /* ... lobby logic ... */ return; }
